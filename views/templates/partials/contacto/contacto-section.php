<section id="contacto" class="bg-dark py-5">

    <div class="container py-5">

      <div class="row">
        <div class="col-lg-6 mx-auto">
          <div class="card rounded-0 border-0">
            <div class="card-body bg-danger">
              <h3 class="card-title text-light text-uppercase text-center">contacto</h3>
            </div>
          </div>
        </div>
      </div>
      
    </div>

    <div class="container-fluid py-4">
      <div class="row">
        <div class="col-lg-4 mx-auto">
          <div class="row">
            <div class="col">
              <a href="{{socialNetworks.whatsapp}}">
                <h5 class="text-center"><i class="fas fa-phone fa-2x mt-4 text-danger"></i></h5>
                <h6 class="text-center text-light text-uppercase">teléfono</h6>
                <h5 class="text-center text-light font-weight-bold">81-17-90-67-96</h5>
              </a>
            </div>
          </div>

          <div class="row">
            <div class="col">
              <h5 class="text-center"><i class="fas fa-envelope fa-2x mt-4 text-danger"></i></h5>
              <h6 class="text-center text-light text-uppercase">mail</h6>
              <h5 class="text-center text-light font-weight-bold">contacto@ferrumsa.com</h5>
            </div>
          </div>

          <div class="row">
            <div class="col">
              <a href="{{socialNetworks.maps}}">
                <h5 class="text-center"><i class="fas fa-map-marker-alt fa-2x mt-4 text-danger"></i></h5>
                <h6 class="text-center text-light text-uppercase">ubicación</h6>
                <h5 class="text-center text-light font-weight-bold">Antonio E. Marcos Lote 5, Colonia Cosmópolis, Apodaca Nuevo León, CP:66612</h5>
              </a>
            </div>
          </div>

        </div>

        <div class="col-lg-4 mx-auto">
          <a href="{{socialNetworks.maps}}">
            <div class="card rounded-0 border-0 bg-transparent">
              <img src="{{ base_url() }}/img/contacto/mapa.jpg" alt="" class="card-img rounded-0 border-0">
            </div>
          </a>
        </div>
      </div>
    </div>
  </section>
